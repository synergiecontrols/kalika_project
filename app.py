...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    "postgresql://synergie_user:synergie1234@localhost/synergie_change"
)
...
def add_data_to_daily_data_count():
    with app.app_context():
        ...
        # Define the start and end time (8 AM to 8 AM)
        start_time = datetime.combine(now.date()-timedelta(days=1), t(8, 0, 0))
        end_time = start_time + timedelta(days=1)

        # Function to query the max count and energy

        # Check if the current time is close to the end of Shift A (e.g., 7:59 PM)
        if now.time() >= t(7,58) and now.time() < t(8,0):
            total_billet_count, total_cobble_count, total_energy_consume = (
                query_max_values(start_time, end_time)
            )
            daily_data_count = Daily_data_count(
                daily_billet_count=total_billet_count,
                daily_cobble_count=total_cobble_count,
                daily_energy_consumption=total_energy_consume,
                date=now.date()-timedelta(days=1),
            )
            db.session.add(daily_data_count)
            db.session.commit()
...
def unapproved_events():
    with app.app_context():
        ...
        if current_user.role in ["planthead", "admin", "operator"]:
            ...
            area_filter = {
                "engineerarea1": "roughing",
                "engineerarea2": "intermediate" ,
                "engineerarea3": "finishing",
...
def handle_event_action(event_id):
    ...
    if current_role in ["engineerarea1", "engineerarea2", "engineerarea3"]:
        if action == "approve":
            if current_role == "engineerarea1" and event.event_area == "roughing":
                event.approved_events = True
                event.approved_by_id_l1 = current_user.id
            elif current_role == "engineerarea2" and event.event_area == "intermediate":
                event.approved_events = True
                event.approved_by_id_l1 = current_user.id
            elif current_role == "engineerarea3" and event.event_area == "finishing":
                event.approved_events = True
                event.approved_by_id_l1 = current_user.id
            else:
            ...
            if int_data[0] == 2:  # If machine stops
                if (
                    ...
                    if cobble_produced:
                        ...
                        if int_data[2] == 4:
                            plc_data_processed["event_area"] = "roughing"
                        elif int_data[2] == 5:
                            plc_data_processed["event_area"] = "intermediate"
                        elif int_data[2] == 6:
                            plc_data_processed["event_area"] = "finishing"
    ...
    while True:
        try:
            if plc.get_connected():
                ...
                energy_cons = snap7.util.get_int(data, 12)
                fuel_cons = snap7.util.get_int(data, 14)
                billet_count = snap7.util.get_int(data, 16)
...
def plc_thread():
    ...
    with app.app_context():
        while True:
            ...
            time.sleep(0.1)  # Add sleep to prevent rapid looping
...
def schedule_task():
    ...
    scheduler.add_job(
        func=add_data_to_daily_data_count, trigger="cron", hour=7, minute=59
    )
    logger.info("Scheduled task to add daily count to daily data count at 07:59.")
    ...
    # Keep the scheduler alive as long as the application is running
    # Comment out atexit if the scheduler should stay active
